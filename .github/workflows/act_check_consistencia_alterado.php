...
//pessoa inclui horário certo das 14 as 17, depois veio e alterou para até as 20
// 2- Verifica se este agendamento já foi cadastrada na mesma hora inicial ou final
    
	$sql = "SELECT agenda_seq,agenda_start_time,agenda_end_time FROM agenda WHERE agenda_sala=". $agenda_sala ." AND ";
	$sql.=	"agenda_seq > 0 AND ((agenda_concluded = 0 OR agenda_concluded = 3) AND ('";
        ...
        $sql.=	"(" . $agenda_seq ." <> agenda_seq AND (agenda_start_time = '". $agenda_start_time . "' OR agenda_end_time ='". $agenda_end_time . "')))" ;
	...
	/*
		...
		while($row = $result_verify->fetch_assoc())
			{ 
				if ($row["agenda_seq"] != $agenda_seq AND ($agenda_start_time == )
