...
{   
	...
	$linha=0;
		...
		while($linha <= 26)
		{
			$sql = "SELECT agenda_concluded, agenda_desc,agenda_seq,time_format(agenda_start_time,'%H:%i') as agenda_time_ini, time_format(agenda_end_time,'%H:%i') as agenda_time_end, prof_name, agenda_start_time, agenda_end_time, client_name, client_seq FROM agenda";
			$sql.= " left outer join profissional on (agenda_prof_seq=prof_seq)left outer join client on (agenda_client_seq=client_seq)";
			$sql .=	" WHERE agenda_client_seq <> 12 and agenda_seq > 0 AND agenda_sala=". $i ." AND";
 				...
 				if ($result2)
 				{
				    ...
				    //echo  '<div class="event_fill" style="height:'. $heightdiv .'px;"> Cliente: '.$row["client_name"] .'<br>Prof: '.$row["prof_name"] . '</div>';
				       ...
				       $strdiv='<div id="'. $div_name . '" style="height:'. $heightdiv .'px;">';
                                       $urlcancel= $path_inpulsecontrol ."/act_del_agenda.php?agenda_seq=". $row["agenda_seq"]. "&agenda_date=". $agenda_start_time;
                                       echo $strdiv;
                                       ?>
                                       
                                       
                                       <a href="<?php echo $urlcancel;?>" onclick="return confirm('Você tem certeza que deseja APAGAR esse agendamento ?');">
                    	...
                    	$strdiv.= substr($row["client_name"], 0,30)  . '/'. $tel_client. '/'. $row["prof_name"]  . '/' . substr($row["agenda_desc"], 0,17) . '</a> </div>';
