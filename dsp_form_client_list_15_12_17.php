			...
			xmlhttp.onreadystatechange = function() 
			{
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
				{
					...
					if(strclients.trim() != "Null")
					{
						...
						document.getElementById("client_profissao").value = clientsArray[12];
						document.getElementById("client_sexo").value = clientsArray[13];
						document.getElementById("client_cidade").value = clientsArray[14];
						// variável com recordcount
						var record_telefone=clientsArray[15];
						
						var radios = document.getElementsByName("client_sexo");
						for (var i = 0; i < radios.length; i++) {
							if (radios[i].value === clientsArray[13]) {
						...
						{	
						console.log("record_telefone: ",record_telefone);
...
echo '<div id="frmclient"><FORM ACTION="act_ins_clients.php" METHOD="POST" id="formclient" name="formclient">';
		...
		$strfield= 'Cidade: <input class="frmclient" type="text" name="client_cidade" id="client_cidade" size="50"';
		...
		$strfield= 'ProfissÃ£o: <input class="frmclient" type="text" name="client_profissao" id="client_profissao" size="30" ';
		$strfield.= 'value="'.$client_profissao.'">';
		echo $strfield;
		
		
		$strfield= 'Sexo: '.$client_sexo;
		$strfield.='<input type="radio" name="client_sexo" id="client_sexo" ';
		if($client_sexo=="M"){
			$strfield.=' checked';
		}	
