...
if (isset($_REQUEST['agenda_seq']))
	{
		...
		if ($result->num_rows)
		{
			...
			$agenda_pvend_seq=$linha["agenda_pvend_seq"];
...
echo '<div id="frmagenda"><b>Dados do Agendamento:</b><br>';
	...
	<FORM ACTION="act_ins_agenda.php" METHOD="POST" id='formagenda' name='formagenda'>
	     ...
	     <?php
	     	...
	     	if ($agenda_sala == 4)
	     		$stroption.='selected';
	     	$stroption.='>Facial</option>';
	     	
	     	$stroption.='<option value= "5"';
	     	if ($agenda_sala == 5)
	     		$stroption.='selected';
	     	$stroption.='>Manicure</option>';
	     	...
	     	$stroption='Profissional que Vendeu:';
	     		
	     	$sql = "Select * FROM profissional WHERE prof_seq > 0 ORDER BY prof_name ";
	     	$result = $conn->query($sql);
	     	$stroption.=' <select class="frmclient" name="agenda_pvend_seq" id="agenda_pvend_seq" required >';
	     		
	     	$stroption.='<option value="">';
	     	while ($linha = $result->fetch_assoc())
	     	{
	     		$stroption.='<option value="'. $linha["prof_seq"]. '"';
	     		if ($linha["prof_seq"] == $agenda_pvend_seq)
	     			$stroption.='selected';
			...
			$strfield= 'Hora Inicial: <input class="frmclient" required min="07:00" max="22:00" type="time" name="agenda_start_time" id="agenda_start_time" size="30"';
			$strfield.= 'value="'.$agenda_start_time.'">';
			echo $strfield;
			
			
			$strfield= 'Hora Final: <input class="frmclient" type="time" required min="07:00" max="22:00" name="agenda_end_time" id="agenda_end_time" size="30"';
			...
			$strfield= 'Lembrete <input type="text" class="frmclient" required name="agenda_log_comments" id="agenda_log_comments" size="40"';
			...
			{
				$qtd_sesao=0;
				// verifica se não foi atribuido pacote ao agendamento ainda, senao dá erro
				if($agenda_secl_seq != "")
				{
					//seleciona as quantidades de sessoes realizadas
					$sql_qtd = "Select count(agenda_secl_seq) as qtd_sessao FROM agenda WHERE agenda_seq > 0 AND agenda_concluded=3 AND agenda_secl_seq=".$agenda_secl_seq;
				
					//echo $sql_qtd;
					$result_qtd = $conn->query($sql_qtd);
				
					while ($linha_qtd = $result_qtd->fetch_assoc())
					{
						$qtd_sesao=$linha_qtd["qtd_sessao"];
					
					}
				}
				
				$strfield= utf8_encode('Quantidade de Sessões'). '<input type="readonly" class="frmclient" name="qtd_sessao" id="qtd_sessao" size="5"';
				$strfield.= 'value="'.$qtd_sesao .'">';
				echo $strfield;
				...
				{
					echo 'Pacote:<select name="secl_seq" class="frmclient" id="secl_seq" onchange="calculaqtdsessoes()" required >';
					// Atribui o código HTML para montar uma tabela
					while ($linha = $result->fetch_assoc())
					{
