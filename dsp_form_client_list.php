			...
			xmlhttp.onreadystatechange = function() 
			{
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
				{
					...
					if(strclients.trim() != "Null")
					{
						...
						//onde conheceu
						document.getElementById("client_indicacao").value = clientsArray[12];
						document.getElementById("client_profissao").value = clientsArray[13];
						document.getElementById("client_cidade").value = clientsArray[14];

					
						if(clientsArray[15]=='F')
						{
							document.getElementById("client_sexo_f").checked = true;
							document.getElementById("client_sexo_m").checked = false;
						}
						else
						{
							document.getElementById("client_sexo_m").checked = true;
							document.getElementById("client_sexo_f").checked = false;
						}
						
						
						// variável com recordcount
						var record_telefone=clientsArray[16];
...
// Ao pressionar o botao envia email, envia email marketing para o cliente atual
function sendinfo()
{
	//alert (document.getElementById("client_seq").value);
	
	if (document.getElementById("client_email").value.length > 0) 
		{
			var xmlhttp = new XMLHttpRequest();
			xmlhttp.onreadystatechange = function() 
			{
	...
	xmlhttp.open("GET", "rotinas/email_marketing_all.php?client_seq=" + document.getElementById("client_seq").value, true);
	xmlhttp.send();
}
</script>
</head>
...
if (isset($_REQUEST['client_seq']))
{
	...
	while ($linha = $result->fetch_assoc())
	{
		...
		$client_cidade=$linha["client_cidade"];
		...
		$client_profissao=$linha["client_profissao"];
		$client_indicacao=$linha["client_indicacao"];
...
echo '<div id="frmclient"><FORM ACTION="act_ins_clients.php" METHOD="POST" id="formclient" name="formclient">';
		...
		$strfield= 'Cidade: <input class="frmclient" type="text" name="client_cidade" id="client_cidade" size="30"';
		$strfield.= ' value="'.$client_cidade.'">';
		echo $strfield;
		...
		$strfield= '<br>Email: <input type="text" name="client_email" id="client_email" size="30"';
		...
		$strfield= 'Sexo: ';
		$strfield.='<input type="radio" name="client_sexo" id="client_sexo_f" ';
		if($client_sexo=="M"){
			$strfield.=' checked';
		}
		$strfield.=' value="M">Masculino';
		
		$strfield.='<input type="radio" name="client_sexo" id="client_sexo_m" ';
		if($client_sexo=="F"){
			$strfield.=' checked';
		}
		$strfield.=' value="F">Feminino<br>';
		echo $strfield;
		
		
		
		$strfield= utf8_encode("<br>Profissão:") . '<input class="frmclient" type="text" name="client_profissao" id="client_profissao" size="30"';
		$strfield.= ' value="'.$client_profissao.'">';
		echo $strfield;
		
		$strfield= utf8_encode("Indicacão:") .' <input class="frmclient" type="text" name="client_indicacao" id="client_indicacao" size="30"';
		$strfield.= 'value="'.$client_indicacao.'">';
		echo $strfield;
